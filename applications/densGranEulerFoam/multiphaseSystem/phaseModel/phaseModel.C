...
:
    ...
    (
        IOobject
        (
            IOobject::groupName("granP", phaseName),
            mesh.time().timeName(),
            mesh,
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
...
{
  Info<<"nuRet"<<endl;
  return nuPtr_().calcNu();
}
Foam::tmp<Foam::volScalarField> Foam::phaseModel::granP() const
{
  Info<<"granPRet"<<endl;
  return nuPtr_().calcGranP();
}
