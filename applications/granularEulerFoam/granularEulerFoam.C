...
int main(int argc, char *argv[])
{
    ...
    while (runTime.run())
    {
		...
		//}
	    //	    srnz = alpha1*Foam::sqrt(2.0)*mag(symm(fvc::grad(U1)))+(scalar(1) - alpha1)*Foam::sqrt(2.0)*mag(symm(fvc::grad(U2)));

	    #include "update_p_IR.H"
	...
	//    	srnz = max(alpha1*Foam::sqrt(2.0)*mag(symm(fvc::grad(U1)))+(scalar(1) - alpha1)*Foam::sqrt(2.0)*mag(symm(fvc::grad(U2))),smallval_sr);
