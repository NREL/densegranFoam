...
Foam::tmp<Foam::volSymmTensorField>
Foam::RASModels::hypoplasticModel::RK4solver2D(volSymmTensorField sigmahypinp) const
{
  dimensionedScalar rk_h = U_.mesh().time().deltaT();

  volScalarField sh11("sh11", sigmahypinp.component(tensor::XX));
  volScalarField sh12("sh12", sigmahypinp.component(tensor::XY));
  volScalarField sh13("sh13", sigmahypinp.component(tensor::XZ));
  volScalarField sh21("sh21", sigmahypinp.component(tensor::YX));
  volScalarField sh22("sh22", sigmahypinp.component(tensor::YY));
  volScalarField sh23("sh23", sigmahypinp.component(tensor::YZ));
  Info<<"RK4solver2D: 1"<<endl;
  volSymmTensorField sigmahyp = this->sigmahypfunc(sh11,sh12,sh13,sh21,sh22,sh23);
  Info<<"RK4solver2D: 2"<<endl;
  ...
  Info<<"RK4solver2D: 10"<<endl;
  sigmahypinp.replace(0, sh11);
  sigmahypinp.replace(1, sh12);
  sigmahypinp.replace(2, sh13);
  sigmahypinp.replace(3, sh21);
  sigmahypinp.replace(4, sh22);
  sigmahypinp.replace(5, sh23);

  Info<<"RK4solver2D: 11"<<endl;
  return tmp<volSymmTensorField>
     (
         new volSymmTensorField
         (
						  ...
						  volScalarField sigmahyp21, volScalarField sigmahyp22, volScalarField sigmahyp23) const
{
  ...
  Info<<1-n_<<endl;
  Info<<max(-tr(sigmahyp)/hs_)<<endl;
  Info<<min(-tr(sigmahyp)/hs_)<<endl;
  Info<<max(-pow(-tr(sigmahyp)/hs_,1-n_))<<endl;
  Info<<min(-pow(-tr(sigmahyp)/hs_,1-n_))<<endl;

  Info<<"sigpow1"<<endl;
  volScalarField powtrsigma ("powtrsigma", pow(-tr(sigmahyp)/hs_,n_));
  Info<<"sigpow2"<<endl;
  volScalarField powtrsigma1n ("powtrsigma1n", pow(-tr(sigmahyp)/hs_,1-n_));
  Info<<"sigpow3"<<endl;
...
Info<<"sigmahypfunc: 23"<<endl;
 ...
 Info<<hs_/max(n_,scalar(1e-1))/max(hi,smallVal)<<endl;
 Info<<beta_<<endl;
 Info<<max(1+ei)<<endl;
 Info<<max(ei/max(voidRatio,smallVal))<<endl;
 Info<<min(ei/max(voidRatio,smallVal))<<endl;
 Info<<max(pow(ei/max(voidRatio,smallVal),beta_))<<endl;
 ...
 volScalarField fs_
  ...
  Info<<max(phidivsigmahyp11)<<endl;
  Info<<max(omegaD21*sigmahyp12)<<endl;
  Info<<max(omegaD12*sigmahyp21)<<endl;
  Info<<max((a1*fs_*(scalar(6.0)*a1*srnzT11 + sqrt(scalar(2.0))*fd_*sigmahypndev11*srnz))/scalar(6.0))<<endl;
  Info<<max((fs_*sigmahyp11*(sigmahyp11*srnzT11 + sigmahyp21*srnzT12 + sigmahyp12*srnzT21 + sigmahyp22*srnzT22))*
                             pow(trsigmahypinv,2))<<endl;
  Info<<max((a1*fd_*fs_*sigmahyp11*srnz)*trsigmahypinv)<<endl;
  Info<<max(srnzT11)<<endl;
  Info<<max(fs_)<<endl;
 ...
 volSymmTensorField Dsigmahyp("Dsigmahyp", rho_*nut_*2*dev(symm(fvc::grad(U_)))/U_.mesh().time().deltaT());
 
 Dsigmahyp.replace(0, Dsigmahyp11);
 Dsigmahyp.replace(1, Dsigmahyp12);
 Dsigmahyp.replace(2, Dsigmahyp13);
 Dsigmahyp.replace(3, Dsigmahyp21);
 Dsigmahyp.replace(4, Dsigmahyp22);
 Dsigmahyp.replace(5, Dsigmahyp23);
...
void Foam::RASModels::hypoplasticModel::correct()
{
  ...
  volSymmTensorField  sigmahyptemp = this->RK4solver2D(sigmaHyp);
  symmTensor smallS = sigmahyptemp[0]*scalar(0.0);
  forAll(sigmahyptemp, i){
    sigmaHyp[i] = min(sigmahyptemp[i],smallS);
  }
