...
Foam::biomassModels::viscosityModels::eulerNonlocalGF::calcNu() const
{
	...
	Info<< "Calculating field g.h_NLGF_1\n" << endl;
	for( int ii = 0; ii < nNormalCell; ii++)
	  {
	    if(alpha_[i]<alphaBoundary){
              break;
            }
	    Info<< "Calculating field g.h_NLGF_2\n" << endl;
	    yc = yc + ii*dy;
	  ...
	  }Info<< "Calculating field g.h_NLGF_3\n" << endl;
	alpha_h[i] = trapsum;
	}
    Info<< "Ended Calculating field g.h_NLGF_4\n" << endl;
    ...
    /*
    volScalarField rho_gh
    (
     "rho_gh",
     rho_*pow(((delfc_/in0_*graind_*srnz_*alpha_)/max((alphaMax_-alpha_),scalar(1.0e-4))),2)
     );
    */



    Info << max(rho_gh) << endl;
    Info << min(rho_gh) << endl;
    ...
    //dimensionedScalar nu_sig("nu_sig", dimViscosity, 1e-1);
    //nu_sig = min(nu_,nu0_);
    /*    
    volSymmTensorField sigmaD
      (
       "sigmaD",
       rho_*(2.0*nu_*tr(symm(fvc::grad(U_))) - (2.0/3.0)*nu_*tr(symm(fvc::grad(U_))))
       );
    */
    volScalarField pres_reg
      (
    ...
    //srnz_Ptr = sqrt(2.0)*mag(symm(fvc::grad(U_)));;
    //srnz_ = sqrt(2.0)*mag(symm(fvc::grad(U_)));;
    /*    
    volScalarField srnz_
        (
         "srnz_",
    ...
    */
    ...
    //    volScalarField srnz_(U_.mesh().lookupObject<volScalarField>("srnz"));
    ...
    volScalarField mu
        (
         "mu",
         //max(min(srnz_/gnz, mu2-delmu), fcs_+delmu)
         // do not limit lower value, JJS 10/5/18
         min(srnz_/gnz, mu2-delmu)
        );
    ...
    fvScalarMatrix gEqn
      (fvm::ddt(g_nlgf) + fvm::div(phi_, g_nlgf) + fvm::SuSp(-fvc::div(phi_), g_nlgf)   
       ...
       - fvm::Sp(t0inv_*delfc_*(fcs_*g_nlgf - srnz_)/(mu2*g_nlgf - srnz_), g_nlgf)
       -fvm::Sp(t0inv_*b_nlgf*graind_*srnz_ * sqrt(rho_/pres_reg), g_nlgf)
       );
...
(
 ...
 const surfaceScalarField& alphaRhoPhi
  ...
  alphaMax_(eulerNonlocalGFCoeffs_.lookupOrDefault<scalar>("alphaMax",1)),
  ...
  pres_(U.mesh().lookupObject<volScalarField>("p")),
  g_(U_.mesh().lookupObject<uniformDimensionedVectorField>("g")),
  p_rgh_(U.mesh().lookupObject<volScalarField>("p_rgh")),
...
)
{
    ...
    eulerNonlocalGFCoeffs_.lookup("alphaMax") >> alphaMax_;
