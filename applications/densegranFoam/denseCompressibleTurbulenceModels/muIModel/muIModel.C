...
void Foam::RASModels::muIModel::correct()
{
  ...
  dimensionedScalar nuC("nuC", dimLength*dimLength/dimTime, scalar(1.84e-05));
  dimensionedScalar rhoC("rhoC", dimMass/dimLength/dimLength/dimLength, scalar(1.5));

  volScalarField StokesN("StokesN",scalar(2.0)*da*da*srnz*rhoS/nuC/rhoC);

  Info()<<"minStokesN = "<<min(StokesN)<<"\n";
  Info()<<"maxStokesN = "<<max(StokesN)<<"\n";
  ...
  nut_ = max(min(muI*pa/rhoS/sqrt(pow(srnz,2)+pow(smallval_sr,2)),nu0_),minNu);
